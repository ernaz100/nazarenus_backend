...
from priorMDM.model.comMDM import ComMDM
...
def load_model_blending_and_diffusion(args_list, device, ModelClass=MDM, DiffusionClass=gd.GaussianDiffusion):
    models = [load_model(args, device, ModelClass)[0] for args in args_list]
    model = ModelBlender(models, [1 / len(models)] * len(models)) if len(models) > 1 else models[0]

    _, diffusion = create_model_and_diffusion(args_list[0], DiffusionClass=DiffusionClass)
