...
from priorMDM.data_loaders.tensors import collate as all_collate, babel_collate, babel_eval_collate, pw3d_collate
from priorMDM.data_loaders.tensors import t2m_collate
from torch.utils.data._utils.collate import default_collate
from diffusion_motion_inbetweening.data_loaders.humanml.data.dataset import collate_fn as sorted_collate
def get_dataset_class(name, load_mode):
    if name == "babel":
        if load_mode == "text_only":
            load_mode = 'train'
        if load_mode in ['gt', 'eval', 'movement_train', 'evaluator_train']:
            from diffusion_motion_inbetweening.data_loaders.humanml.data.dataset import BABEL_eval
            return BABEL_eval
        elif load_mode == 'train':
            from priorMDM.data_loaders.amass.babel import BABEL
            return BABEL
        else:
            raise ValueError(f'Unsupported load_moad name [{load_mode}]')
    elif name == "humanml":
        from diffusion_motion_inbetweening.data_loaders.humanml.data.dataset import HumanML3D
        return HumanML3D
    elif name == "pw3d":
        from diffusion_motion_inbetweening.data_loaders.humanml.data.dataset import PW3D
        return PW3D
    else:
...
def get_collate_fn(name, load_mode='train'):
    if load_mode == 'gt':
        from diffusion_motion_inbetweening.data_loaders.humanml.data.dataset import collate_fn as t2m_eval_collate
...
def get_dataset(name, num_frames, split='train', load_mode='train', batch_size=None, opt=None, short_db=False, cropping_sampler=False, size=None):
    ...
    if name in ["humanml", "pw3d"]:
        dataset = DATA(mode="train",split=split, num_frames=num_frames, load_mode=load_mode, size=size,use_abs3d=True)
    elif name == "babel":
        from priorMDM.data_loaders.amass.transforms import SlimSMPLTransform
