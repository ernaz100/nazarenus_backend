...
def plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
    ...
    def plot_target_pose(target_pose, frame_idx, cur_root_loc, used_colors, kinematic_tree):
        ...
        for target_frame in frame_idx:
            # Skip if target_frame is outside the valid range
            if target_frame >= len(target_pose):
                continue
    ...
    def update(frame_idx):
        ax.clear()
        init()  # Reinitialize the plot settings
        
        ax.view_init(elev=120, azim=-90)
        ax.dist = 7.5

        plot_xzPlane(MINS[0] - trajec[frame_idx, 0], MAXS[0] - trajec[frame_idx, 0], 0, MINS[2] - trajec[frame_idx, 1],
                     MAXS[2] - trajec[frame_idx, 1])

        plot_obstacles(trajec[frame_idx])
        plot_ground_target(trajec[frame_idx])
        ...
        for i, (chain, color) in enumerate(zip(kinematic_tree, used_colors)):
            if i < 5:
                ...
                linewidth = 2.0
            ax.plot3D(data[frame_idx, chain, 0], data[frame_idx, chain, 1], data[frame_idx, chain, 2], linewidth=linewidth,
                      color=color)

        if traj_only:
            ax.scatter(data[frame_idx, 0, 0], data[frame_idx, 0, 1], data[frame_idx, 0, 2], color=color)

        def plot_root_horizontal():
            ax.plot3D(trajec[:frame_idx, 0] - trajec[frame_idx, 0], np.zeros_like(trajec[:frame_idx, 1]), 
                      trajec[:frame_idx, 1] - trajec[frame_idx, 1], linewidth=2.0,
                      color=used_colors[0])

        plot_root_horizontal()
        
        # Filter gt_frames to only include valid frame indices
        valid_gt_frames = [f for f in gt_frames if f < len(target_pose)]
        plot_target_pose(target_pose, valid_gt_frames, data_copy[frame_idx, 0, :], colors_blue, kinematic_tree)
